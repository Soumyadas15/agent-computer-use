...
impl AtspiContext {
    pub async fn new() -> Result<Self> {
        let session = Connection::session().await.map_err(|e| plat_err(&format!(
            "failed to connect to session bus: {e}"
        )))?;
    ...
    pub async fn element_to_node(
        ...
        let role = proxy.get_role().await.map(map_role).unwrap_or(Role::Unknown);
