...
mod real {
    ...
    impl Platform for LinuxPlatform {
            ...
            match action {
                ...
                Action::Screenshot { path, app } => {
                    let save_path = path.clone().unwrap_or_else(|| {
                        format!("{}/agent-click-screenshot.png", std::env::temp_dir().display())
                    });
        ...
        async fn windows(&self, app: Option<&str>) -> Result<Vec<WindowInfo>> {
            ...
            for child in &root.children {
                if matches!(
                    child.role,
                    agent_click_core::node::Role::Window | agent_click_core::node::Role::Dialog
                ) {
    ...
    async fn resolve_point(
        ...
        coordinates: Option<agent_click_core::node::Point>,
    ) -> Result<agent_click_core::node::Point> {
