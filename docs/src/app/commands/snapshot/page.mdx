...
```bash
agent-computer-use snapshot [OPTIONS]
```
...
```bash
agent-computer-use snapshot -a Calculator -i -c        # interactive + compact (best for AI)
agent-computer-use snapshot -a Safari -i -c -d 8       # deeper tree
agent-computer-use snapshot -a Music                   # full tree (verbose)
```
...
```bash
agent-computer-use click @e3          # clicks "7"
agent-computer-use get-value @e1      # reads "Delete" button info
```
...
- Refs are cached in `~/.agent-computer-use/refs.json` with a 5-minute staleness warning
