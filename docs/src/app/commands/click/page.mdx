...
```bash
agent-computer-use click [SELECTOR] [OPTIONS]
```
...
# By selector
agent-computer-use click 'name="Login"' -a Safari
...
# Right-click
agent-computer-use click @e5 --button right

# Coordinate click (when no selector works)
agent-computer-use click --x 500 --y 300 -a Finder
...
```bash
agent-computer-use click @e5    # JS element.click() via CDP — fully headless
```
...
- Check with `agent-computer-use get-value @e5` if unsure whether an element is clickable
