...
agent-computer-use get-value @e7
agent-computer-use scroll-to @e42
agent-computer-use drag @e5 @e10 -a Finder
```

## Ref lifecycle

1. **Created** when you run `agent-computer-use snapshot`
2. **Cached** in `~/.agent-computer-use/refs.json`
...
Each ref stores the element's path in the tree (e.g., `[0, 1, 3, 2]`). When you use `@e5`, agent-computer-use walks the tree using this path — O(depth) lookup, much faster than searching.

If the path becomes stale (element moved), agent-computer-use falls back to matching by role + name + id.
